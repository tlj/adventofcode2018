...
func getCurrentGuard(line string) (int64, error) {
	pattern := regexp.MustCompile(`Guard #(\d+) begins shift`)

	found := pattern.FindAllStringSubmatch(line, 1)

	if len(found) == 0 {
		return 0, fmt.Errorf("Line did not match pattern: %s\n", line)
	}

	guard, err := strconv.ParseInt(found[0][1], 10, 64)
	if err != nil {
		return 0, err
	}
...
func getMinutePastMidnight(line string) (int64, error) {
	pattern := regexp.MustCompile(`00:(\d+)`)
	...
	minute, err := strconv.ParseInt(found[0][1], 10, 64)
	if err != nil {
		return 0, err
	}
...
type guard struct {
	ID int64
	sleepMinutes int64
	fellAsleepLast int64
	asleepAtMinutes map[int64]int64
}

func (g *guard) Sleep(minute int64) {
	g.fellAsleepLast = minute
}

func (g *guard) WakeUp(minute int64) {
	g.sleepMinutes += minute - g.fellAsleepLast
	for i := g.fellAsleepLast; i < minute; i++ {
		g.asleepAtMinutes[i]++
	}
}

func (g *guard) MostSleepyMinute() int64 {
	var most int64
	var mostMin int64
	...
	for min, c := range g.asleepAtMinutes {
		if c > most {
			most = c
			mostMin = min
		}
...
func main() {
	...
	var currentGuard int64
	guards := make(map[int64]*guard)

	for _, s := range lines {
		newGuard, err := getCurrentGuard(s)
		if err == nil {
			currentGuard = newGuard
			if _, ok := guards[currentGuard]; !ok {
				guards[currentGuard] = &guard{ID:currentGuard, asleepAtMinutes:make(map[int64]int64)}
			}
			continue
		}
		currentMinute, err := getMinutePastMidnight(s)
		if err != nil {
			fmt.Printf("Error parsing minute (%s): %v", s, err)
			continue
		}

		if strings.Index(s, "falls asleep") > -1 {
			guards[currentGuard].Sleep(currentMinute)
		}
		if strings.Index(s, "wakes up") > -1 {
			guards[currentGuard].WakeUp(currentMinute)
		}
	}
	...
	for _, guard := range guards {
		if guard.sleepMinutes > mostSleepyGuard.sleepMinutes {
			mostSleepyGuard = guard
		}
	}

	fmt.Printf(
		"Most sleepy guard: %d, with %d minutes. Most sleepy minute: %d on %d days.\n",
		mostSleepyGuard.ID,
		mostSleepyGuard.sleepMinutes,
		mostSleepyGuard.MostSleepyMinute(),
		mostSleepyGuard.asleepAtMinutes[mostSleepyGuard.MostSleepyMinute()],
		)
	fmt.Printf("Part1: %d\n", mostSleepyGuard.ID * mostSleepyGuard.MostSleepyMinute())

	var mostAsleepAtSameMinuteGuardMinute int64
	var mostAsleepAtSameMinuteGuardMinuteCount int64

	mostAsleepAtSameMinuteGuard := &guard{}

	for _, guard := range guards {
		for min, c := range guard.asleepAtMinutes {
			if c > mostAsleepAtSameMinuteGuardMinuteCount {
				mostAsleepAtSameMinuteGuardMinute = min
				mostAsleepAtSameMinuteGuard = guard
				mostAsleepAtSameMinuteGuardMinuteCount = c
			}
		}
	}

	fmt.Printf("Guard %d most asleep at minute %d. Days: %d\n", mostAsleepAtSameMinuteGuard.ID, mostAsleepAtSameMinuteGuardMinute, mostAsleepAtSameMinuteGuardMinuteCount)
	fmt.Printf("Part2: %d\n", mostAsleepAtSameMinuteGuard.ID * mostAsleepAtSameMinuteGuardMinute)
}

var testInput1 = `[1518-11-01 00:00] Guard #10 begins shift
...
var input = `[1518-10-05 00:10] falls asleep
